...
export class UtilService {
  ...
  mutex:boolean = false;
  dialogRef:any;
  constructor(private http: Http, private router: Router, private apiData: ApiData) {
    this.mutex = false;
   }
  ...
  callPostApi(apiurl: string, body: any, options?) {
    ...
    if(this.isInValidSession()){
     this.router.navigate(["/login"])
      ...
      catchError(
        (error: Response) => ((error.status == 401 || error.status == 404) && apiurl != (this.apiData.BASE_URL + this.apiData.URL_LOGIN)) ? this.handleErrorResponse(error.json()) : Observable.throw(error.json())
